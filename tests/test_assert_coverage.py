...
5. **非测试函数的辅助函数里的 assert 不在表内。** 对比表只收集名字以 ``test`` 开头的函数
   (辅助函数不属于任何单个 test item)。当前 `tests/` 里的辅助函数(`_rendered_html` /
   `_scan_targets` / `_string_literals`)一条 assert 都没有,所以这条盲区今天没有实际影响;
   但一旦有人把断言搬进辅助函数,那行 assert 就悄无声息地脱离了本检查。
