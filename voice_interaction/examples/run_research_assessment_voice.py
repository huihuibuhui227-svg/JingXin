...
使用方法：
    python -m voice_interaction.examples.run_research_assessment_voice
"""
...
def main():
    """主函数：运行科研评估"""
    print("=" * 60)
    print("JingXin 科研评估系统（语音识别版）")
    print("=" * 60)
    print("ℹ️  系统会自动识别您的语音回答")
    print("🛑 输入 'quit' 退出程序\n")

    # 初始化组件
    research = ResearchAssessmentPipeline()
    tts = TTSPipeline()
    ...
    # 开始评估
    while True:
        question = research.get_next_question()
        if not question:
            print("\n" + "=" * 60)
            print("评估已结束！正在生成评估报告...")
            print("=" * 60)
            break
    ...
    print("评估结果")
    print("=" * 60)
    result = research.get_comprehensive_evaluation()
    print(result)

    # 保存日志
    log_path = research.save_log()
    print(f"\n✓ 评估报告已保存至: {log_path}")
