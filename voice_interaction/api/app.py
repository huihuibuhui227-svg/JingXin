...
# 存在的理由(把素材留下来)。
#
# 为什么是 2 GB 而不是先写的 512 MB —— 按 2026-09-25 的 **R5 实测**反推:
# Windows Edge 153 下 `video/webm;codecs=vp8,opus` @1280x720 是
# **1722 KB / 10 秒 = 172 KB/s**。512 MB 只够 ≈ **50 分钟**,而一场面试
# 连着录(含思考时间)撞上是完全可能的 —— 撞上的后果不是"少存一段",
# 是**这一场什么都没存**。2 GB ⟹ ≈ 3.4 小时,任何单场会话都够不着;
# 而"一个请求不许填满盘"这个初衷仍然成立(127 GB 远大于 2 GB)。
# 上限本身有钉子:`tests/test_m26_review_hardening.py` 里那条按实测速率算够不够一小时。
