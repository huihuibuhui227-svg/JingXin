...
class Visualizer:
    ...
    def draw_arm_result(
        self,
        img: np.ndarray,
        arm_result: Optional[Dict[str, Any]],
        ...
        if not isinstance(arm_result, dict):
            return self.put_chinese_text(
                img, f"{label}数据不可用", position, color=(0, 0, 255)
            )

        try:
            # 第一行：手臂评分
            score = arm_result.get('arm_score', 0.0)
            text1 = f"{label}评分: {score:.1f} / 100"
            img = self.put_chinese_text(
                img, text1, position,
                color=self.config['colors']['info']
            )
            ...
            # 第二行：抖动数据
            wrist_jitter = arm_result.get('wrist_jitter', 0.0)
            elbow_jitter = arm_result.get('elbow_jitter', 0.0)
            text2 = f"  手腕抖动: {wrist_jitter:.4f}  肘部抖动: {elbow_jitter:.4f}"
            img = self.put_chinese_text(
                img, text2,
                (position[0], position[1] + 35),
            ...
            # 第三行：角度和稳定性
            angle = arm_result.get('arm_angle', 0.0)
            stability = arm_result.get('arm_stability', 0.0)
            text3 = f"  手臂角度: {angle:.1f}°  稳定性: {stability:.2f}"
            img = self.put_chinese_text(
                img, text3,
                (position[0], position[1] + 70),
                color=self.config['colors']['text']
            ...
            print(f"⚠️ 手臂结果绘制异常: {e}")
            img = self.put_chinese_text(
                img, f"{label}结果显示错误", position, color=(0, 0, 255)
            )
