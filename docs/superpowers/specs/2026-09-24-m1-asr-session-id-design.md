...
- **样本量门槛(与上面的长度下限是两件事,别混)**:本指标过证据门 G3 的门槛登记在
  `report_frontend/evidence_thresholds.json` 的 `thresholds.density` = **5**(带
  `_provisional: true` 与依据),**不是**默认的 `_default_n_valid` = 10。理由:本指标
  **每段回答一个样本**,而默认的 10 当初是给**帧级**列设的(那种列一场有上千帧);
  题库每场只有 8 题,答满全场也才 8 段 —— 即 **10 段不可达**。一个永不达标的门槛不等于
  更严的诚实门,它等价于该指标永久不可用(永远显示「有效样本不足」),M1 的核心验收判据
  因此永不可达。暂用 5(≈ 半场答题量),M5 标定到位后替换。
  - 门槛计的是**真正带值**的行数(特征引擎的 `connective_density_sample_size`):回答
    过短时密度列是空的(§8「不出值、不写 0」),那些行**不计入** —— 否则门槛会被一行
    都不带的行放行(最终审查 I1)。
...
4. **单段回答会被 G2 拦下**:`connective_density_std` 在只有一个样本时为 0,证据门 G2(非常量)会把它判为「本次会话内无变化」。**这是设计使然,不是 bug** —— 验收会话需 **≥2 段回答**才能满足 G2;而要让该槽真正**过门出分**,还要满足 G3 的样本量门槛 **≥5 段**(见 §6.5 的样本量门槛与 §10 的验收门)。
...
**验收门(方案 A,需你配合录一场)**
1. 录一场会话(**≥5 段**回答)—— G2 只要求「会话内有变异」,≥2 段即可满足;但要真正
   **过门出分**还得满足 G3 的样本量门槛 **5**(见 §6.5)。少于此数该槽会被按设计拦下,
   不是 bug。8 段回答的内容必须**各不相同**(同一段音频重复提交 → 每行密度相同 →
   `_std = 0` → G2 按设计判「本次会话内无变化」)
