...
## 合并后的收尾(2026-09-24)与一次控制器操作失误

**已完成**:合并 `9c8d690`;`.gitignore` 由 UTF-16 修回 UTF-8(此前**所有**忽略规则静默失效)+ 停止跟踪 64 个 `.pyc`;`experiments/results/`(1.1 GB / 10,128 文件)入忽略;忽略规则改 `**/data/output/`(含斜杠的规则只能锚定根目录,故 `report_frontend/data/output/` 一直未被忽略);清 42 份修复前旧报告(其中 4 份曾被跟踪);清 1.3 GB 旧门 4 目录(新目录 56 MB);**文档入库**(此前 `docs/` 下只有 1 份被跟踪);**`experiments/duration_audit/` 入库** —— 防线脚本 `reaggregate_normalized.py` 所在的整个 `experiments/` 此前一个文件都没入库,误删即永久丢失。

**⚠️ 控制器失误(记录在案):** 提交 `4479f9c`(本意只提 `experiments/duration_audit/`)**把子智能体当时已暂存的 `tests/test_assert_coverage.py` 一并卷了进去**。原因:`git commit` 提交的是**整个暂存区**,不只是我刚 `git add` 的路径 —— 我在子智能体暂存后、提交前跑了 `git commit`。**内容无损**(文件在、字节一致、51→53 个测试全过),仅该提交的信息未提及此文件。修它需改写历史(rebase 会连带重写其后 3 个提交,含子智能体的),**控制器判断不值得**(仓库未推送、内容正确),故记录而不改写。教训:`git commit` 前若可能有并发暂存,应先用 `git diff --cached --name-only` 核对暂存区。

**后续工作复审结果**:`tests/test_assert_coverage.py` 与 §10 条目 —— **Quality: Approved**,0 Critical、0 Important、**8 Minor**。

复审者**把 checker 的四个守卫逐个弄响**来验证(包括复现它声称修掉的那个假绿:monkeypatch 掉 nodeid 映射后 `n_modules=0` 但 `unmapped` 守卫变红),并独立在仓库外造了个合成模块验证真能抓到死断言;确认自测**非恒真**(用 `ast.unparse` 独立重算行号 + 精确集合相等);逐条核了 docstring 里 7 条盲区是否属实(GH#5 已核:三个 helper 里确实没有 assert)。

**deferred minor(不修,记档):**
1. `:242` 的 `n_assert_lines > 0` 是名义下限(112 vs 0),删掉断言仍会全绿 —— 属已披露盲区,非缺陷。
2. `:102` `cls = parts[-2]` 只处理一层类名;嵌套类会落进 `unmapped` 报错(是 fail-loud,不是假绿),将来若用嵌套测试类需改。
3. docstring 盲区 4 说扫描范围"只有 `tests/test_*.py`",实际 `rglob` 会扫子目录 —— 措辞不准。
4. docstring 盲区 5 断言"今天三个 helper 里没有 assert" —— 关于**其他文件内容**的声明会随代码漂移而烂掉。
5. 设计文档 `:552` 有一句是**控制器/实现者的推论而非来源**:来源只说"MIT 与 ChaLearn 许可尚未核实",没说"核实前不得进上线路径" —— 该条目其余部分都用「**推论:**」显式标注,这句没有。
6. `:552`「这两条线」指代歧义(两条前置 vs 两条数据集线)。
7. 「样板」可能被读成"合规"而非"诚实的登记台账"(该文件里另有 8 条 `legacy_arbitrary` 无依据的因子)。
8. 该条目 601 字符,是 §10 其余条目(48–145 字符)的 4 倍;若嫌密度不齐,可拆成 7 与 8 两条。
