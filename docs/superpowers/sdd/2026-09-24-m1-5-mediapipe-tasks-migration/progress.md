...
### 修复波次:complete (commits 8cbe149..e462d16,复审 6/6 ADDRESSED) + N1 补修 (ca0477f)

范围受限复审判定:I1 / I2 / I3a / I3b / I4 / M9 **全部 ADDRESSED**,并逐条做了变异验证
(如去掉探测器缓存 → 只有那条测试红;`if not path.exists()` → `if False` → 精确红)。

**复审抓到的新破坏,以及控制器的裁决:**

- **Ruling: N1 立即修(不是记档了事)。** — 复审发现**我修 I1 时引入**了一个回归:
  `concurrent.futures` 不报告"没人取回的异常",所以 `_CLOSER.submit(detector.close)`
  里抛的东西**完全静默**;而 `.close()` 只在成功后才清 `_landmarker` → **native 句柄静默泄漏**
  —— 正是 spec §6.3 加 close() 要防的东西。改回同步时失败是看得见的(端点 500)。
  **修法**:`add_done_callback(_log_close_failure)` + `logger.error(exc_info=...)`。
  TDD:先写测试看它红(实测 `assert []`,一条日志都没有)→ 修 → 反向复现(去掉回调精准红)。
  测试 198 → **199**。— 代价:多一个回调;不修的代价:静默泄漏 + 这个项目最恨的那类失败回归。
- **Ruling: N3 的更正写进本账本,不改报告原文。** — 修复波次报告里"atexit 会 join 后台
  close"那句推理**不成立**(复审实测:4 个真探测器提交后进程 5.67s 后死亡,只跑完 1 个 close,
  其余被打断)。结论无害(OS 回收),但推理要改口。**报告是"当时发生了什么"的记录,
  不去改写它;更正写在这里。**
- **N2(Minor,记档不修)**:`max_workers=1` 串行 + 每次 5.0s ⇒ 句柄释放窗口 = 队列深度 × 5s
  (实测 2 会话一次 `/reset` 后后台要 20.0s 才放完,请求不等)。将来真有句柄压力再调线程数。
- **N4(已修)**:计划文件补了一节"修复波次之后这些名字已经变了"(`close_detached` /
  `_wait_until` / 新增的三组钉子),免得照原文重跑写出假红测试。

### ★ 控制器的一次未遂事故(记档,必须留痕)

归档 T7 证据时,我把 `/tmp/t7_a*.json`(`answer_audio` 的响应)拷进了 `docs/superpowers/sdd/`
—— 而那些响应里有 **`recognized_text`,就是识别出的原句**。这正好违反 "原句不进仓库"
(spec D2),也就是我半小时前刚在 T7 里验过的那条不变量。
**发现后立刻删除并复核:仓库内 0 命中;那批文件从未进入 git(删除时仍是未跟踪状态),无历史残留。**
教训:**"给报告/证据留档"这类动作必须先把内容当作不可信检查一遍** —— 我这次是凭"它是验收输出"
就拷了,没看里面装的是什么。同理,`answer_audio` 的响应**按设计**就带原句(客户端要显示识别结果),
所以**任何形式的响应抓取都不能进仓库**。

### 控制器执行期发现(记档,不属于 M1.5 范围)

- **deferred(2026-09-24 T7 实跑发现):连接词密度是在 ASR 输出上算的,ASR 质量直接进指标。**
  实测六段里 `ans4` 与 `ans6` 的识别结果**开头带垃圾**:
  `ans4: 在不双一一一一一一一在在在在在在在在在在在在的的的的在的一的在`(后接真句子)、
  `ans6: 二零一零一一一一一一一一一二零的一的。`(后接真句子);全程另有错字
  ("动物态学习"←多模态、"解拆开"←拆开、"负样门采集"←负样本采样、"动模态"←多模态)。
  垃圾前缀**撑大字数 → 压低密度**(ans4 算出 0.885 而非稿子设计的 1.2;ans6 6.25 而非 7.3)。
  **含义:这个数字量的是"识别出来的文本",不是"被访者说了什么"。** 不阻塞 T7(四条判据全过、
  密度标准差仍 >0),但报告若把它当"话语结构"的证据,要写明这一层传递链。
  — 可能的缓解(未做):录前静音段裁剪、或换非流式/带 VAD 的识别路径。
